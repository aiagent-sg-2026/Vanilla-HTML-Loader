...
A zero-dependency gallery of **888 loading animations** across 17 categories. Browse them live, tune size, speed, accent colour and per-loader application state, then copy production-ready HTML, CSS and JavaScript you can paste straight into any page.
...
Loader Studio 是一个加载动画画廊，**复制出去的代码零依赖**（工作室本身用 Vite 构建），收录 **888 个动画**，分 17 个分类。浏览、调整（尺寸/速度/主色/应用状态）后可直接复制可用于生产的 HTML、CSS 和 JavaScript。
