...
- 585 loaders in 15 categories exposed from `loaders/index.js`; category filter row is generated from the registry (`All` + unique categories).
...
- `qa/snippet-paste-smoke.html`: pastes every loader's combined snippet into a blank page and asserts the overlay mounts and animates; supports `?autorun=1`, `&limit=`, `&category=`, and exposes `window.__smokeResult`. A full 585-loader run takes about 6 seconds.
